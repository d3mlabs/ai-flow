    ...
    class Learn
      ...
      def shared_rubric
        <<~RUBRIC.strip
          ...
          WIDE ANGLE: this pass is one sample, seen from inside. Before drafting, step back and look for the pattern behind the instance in whatever evidence is at hand — recent commits and PRs, the existing index, open learning drafts. When the same friction recurs, capture the class, not the instance. Then ask where the cause lives: a failure that surfaced here but is owned by another repo's code or tooling is that repo's FIX line, not this repo's coping rule.
      ...
      def in_worktree(branch, refine:, &blk)
        ...
        Dir.mktmpdir("ai-flow-learn-", @executor.workspace_base) do |dir|
          # Shared while still empty: everything populated inside inherits
          # the agent-shared group via setgid (plans#26; no-op unisolated).
          # The base comes from the executor too — the agent user must be
          # able to traverse to the leaf (nil = mktmpdir default).
