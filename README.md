...
| `/edit <instruction>` | issues + PRs | On a plan issue: rewrites the quoted section (whole document when unscoped), PATCHes the body through a guarded sync, and appends a blockquoted result panel to the command comment: a header line with a text-fragment backlink, plus collapsed "Word diff" (word-level `<ins>`/`<del>` prose, re-rendered mermaid) and "Source diff" (colored unified diff) sections. On a code PR: applies the instruction on the PR branch, commits, pushes, and appends the commit link. |
