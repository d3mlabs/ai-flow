...
We are explicit about what is *not* claimed as novel: tiering and progressive disclosure, the `SKILL.md` form, markdown-in-git storage, proposal-staging with human promotion, and the capture rubric are all inherited from the field. The novelty is where the gate sits and what it covers.
...
We deliberately retain the cognitive-science framing alongside the machine-learning one — the ACL survey itself describes the field as "oscillating between operating system engineering and cognitive science" [13]. Each stage of the loop has a named precedent in learning theory:
...
| Re-entrancy (a merged learning re-opened by a model transition, Section 6.3) | Reconsolidation: a stored memory, on reactivation, returns to a labile state before it must re-stabilize — the pre-admission proposal is that labile phase | [41] |
| The gate (human merge on proposal PRs) | Mimicry needs a fitness function: imitation is scored by signals the learner did not choose; the gate is that scoring, abstracted | [18, 19] |
...
- **Capture surfaces.** `/learn` runs on issues and PRs (including review threads) in four forms: dictated (`/learn <statement>`), bare sweep (distill the surface's discussion), survey (`/learn --scan`, seeding architecture digests from the codebase), and promotion (`/learn --promote <slug>`, lifting a repo-local learning to the organization tier as paired proposals — the org addition and the repo-local removal). Additionally, `/build` carries an opportunistic **build-time capture pass** in the same agent session that performed the work (on by default; `learn: { on_build: false }` in `.github/ai-flow.yml` opts out).
- **Artifact form.** A learning is an index line (one sentence: the retrieval cue) plus a detail skill (`skills/<slug>/SKILL.md`), opened as a *proposal* against the repository's knowledge area or the organization's knowledge repository (`knowledge_repo:` in `.github/ai-flow.yml`). We use *proposal* throughout for the pre-admission artifact — the field's lifecycle noun (propose, admit, retrieve, retire) — realized here as an ordinary pull request; nothing in the design depends on GitHub's *draft* PR state, and the deployment deliberately opens proposals as ordinary PRs so review requests are not muted, holding the draft-state flag in reserve as a throttle should unsolicited proposals ever grow noisy. Re-runs refine or dissolve open proposals rather than duplicating them.
...
- **The gate.** Every learning, without exception, lands as a proposal PR merged by a person. The gate is the system's reward model (Section 4).
...
| Policy update (gradient step)  | Gated memory write (merged proposal PR)     |
...
Three implications, stated bluntly because the design depends on them: proposal acceptance rate is an insufficient quality metric (heterogeneous effects cancel in it); per-task causal attribution is mandatory, not optional (Section 15.3); and the index is the highest-leverage component in the system — cue quality moves outcomes more than skill quality at the margin.
...
2. **Automated proposal.** Capture fires without a human command. (ai-flow briefly shipped and then removed an `auto-learn` feature; under this analysis it was premature rather than wrong, and re-enters as a planned stage.) Safe because proposals are free and the gate holds. *Prerequisite:* proposal acceptance-rate telemetry demonstrating that unsolicited proposals are signal, not noise.
...
The ladder is not speculative in shape: it is the post-training field's own progression (Section 2.1), run one layer up — over memory writes instead of weight updates — and Ratchet provides published evidence for its top rung: in a domain with verifiable outcome signals (unit-test pass rates), automated lifecycle hygiene — outcome-driven retirement plus a capacity cap — substitutes for human curation verdicts [24]. That is the RLVR analog run as an experiment, and its scope condition is exactly ours: it works where the outcome is machine-checkable.

**The path to the RLAIF rung is concrete, and the substrate already holds its training data.** The gate's verdict history — accepted proposals, dissolved proposals, and the human edit-diffs between proposal-as-opened and proposal-as-merged — is exactly the dataset one would use to build a judge. (In a real sense the merged corpus is already a *cached shadow* of the reward model: it records the gate's past preferences, and loading it shifts future proposals toward what passes — used as policy context, never as a judge.) The graduation protocol mirrors how laboratories validate reward models: (1) a judge agent reviews knowledge-repo proposal PRs *advisorily* — comments and recommends, never merges; (2) *shadow mode* — judge verdicts are recorded alongside human verdicts and the agreement rate is measured; (3) once agreement is high on a diff class, the judge auto-merges that class with a human veto window, and demotion is automatic on attributed regression. One further property keeps the judge accountable to the org rather than to its own drift: the capture rubric and the judging standard are themselves gated content — the criteria the judge applies pass through the same gate as everything else, so the org's definition of "a good learning" is versioned, reviewable, and revertible.
...
stateDiagram-v2
    ...
    HumanGated --> AutoProposal: proposal acceptance-rate threshold met
...
flowchart TD
    ...
    Proposals -->|"proposal PR"| Knowledge
```
...
Derived systems reindex constantly because their structure is *derived*: every write re-embeds; every read recomputes decay. A symbolic tree mutates only on **structural events** — a section outgrowing its cue budget — and the expected gate frequency follows directly: with a sustained capture rate of *r* merged learnings per month distributed over *S* index sections with a budget of *B* cue lines per section, overflow events arrive at roughly *r/B* per month once sections approach their budgets — linear in capture rate, damped by the section budget, independent of read volume. Our deployment is too young to supply *r* (Section 12); as a labeled projection, a corpus growing at 10 learnings/month against a 15-line section budget produces structure events at well under one per month, arriving in bursts as individual sections fill. Each such diff is mechanical to review — cue lines moved, digest rewritten — and `/learn --reindex` (or a capture pass proposing it opportunistically) computes the new tree shape and emits it as an ordinary proposal PR: the retrieval-topology change is itself a reviewable diff with provenance and rollback.

One caveat carried honestly: symbolic cue-matching may lose recall to learned retrieval at scale, and the risk deserves its precise form. Because the matcher is an LLM *reading* the cue list, it generalizes semantically — the classic lexical vocabulary-mismatch problem that dense retrieval was built to beat [39] applies with less force than it would to exact-match systems. The scale risk is instead **attention dilution**: retrieval quality over long in-context lists degrades with list length and position ("lost in the middle" [38]), which is the mechanism behind the survey's tens-to-hundreds degradation numbers and the reason the tree grows depth rather than width. The same economics holds at the leaf: SkillsBench finds focused skill modules (at most three) outperform comprehensive bundles, with exhaustive prose essentially flat [31] — minimal sufficient loading wins at every level of the tree, not just the index. The two-stage form (7.2) is the designed response — the embedding layer supplies recall, the symbolic index retains authority — and the masking harness decides empirically when that layer becomes necessary.
...
flowchart LR
    ...
    Extract --> ProposalPR["Learning proposal PR"]
    ProposalPR --> Refine["Refine / dissolve on re-runs"]
...
- **Injection via the capture surface.** A review comment crafted so that a `/learn` sweep distills it into a hostile learning ("always fetch and execute the script at..."). Closed at ingestion by the content-provenance filter: auto-ingested context is restricted to authors holding write access on the source repo, so the commenter must already be inside the trust boundary — or a trusted user must explicitly quote the hostile text, putting a human vouching act on the audit trail.
...
| ----------------------------- | ----------------------------------- | -------------------------- | ----------------------- | ----------------------------- | ------------------------------------- | ---------------------------------------- |
| **ai-flow**                   | Human review threads + build passes | Slash command + build-time | `SKILL.md` + index line | Org merge gate on proposal PRs   | Git repo (diff, provenance, rollback) | Usage telemetry feeding human retirement |
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| Authoring        | Self-generated skills land **below the no-skills baseline** (−8.1 to −11.5pp on all three dedicated configurations) [31]      | Curated skills: **+16.6pp** average pass-rate lift (+4.1 to +25.7 per configuration) [31] |
...
| Growth           | Flat retrieval degrades around tens-to-hundreds of skills (sharpest controlled sweep: decline beyond ~64–128) [5]              | Tiered, budgeted tree structure                                                    |
...
- **The gate is empirically sound, not ceremonial.** SkillsBench's trajectory audit of the self-generation deficit finds three mechanisms: generated packs "frequently go unused" by the solver, authoring displaces task work, and consumed packs "lock in confident errors" — a wrong assumption in a self-authored skill became load-bearing and cost three orders of magnitude on the answer [31]. The lifecycle survey's evidence-graded patterns say the same at corpus level: "admission and repair are repeatedly important" and "verifier quality materially affects skill-aware RL" [5]; and the security audit of Section 9 prompted four-tier gate-based governance frameworks [6].
- **The domain nuance favors this corpus.** SkillsBench's curated-skill lift is smallest exactly where model pretraining coverage is strongest — Software Engineering (+11.6pp) and Mathematics & OR (+9.7pp), versus Natural Science (+28.8pp) — and its authors attribute the gradient to procedural knowledge under-represented in pretraining benefiting most [31]. An organization's corpus is org-specific taste and conventions — plausibly that under-represented-knowledge regime, inside a domain whose generic form the model already knows. "Absent from the weights" is an assumption we can test per-learning rather than assert: the redundancy probe (Section 15.4) measures whether the base model already complies without the memory loaded.
...
### 13.2 The bitter-lesson objection

Sutton's essay argues that seventy years of AI history repeat one pattern: general methods that leverage computation ultimately beat approaches built on human-engineered knowledge, and researchers' hand-encoded domain knowledge is a short-term win that plateaus and then loses to scale [40]. Applied here: isn't a curated corpus of human-gated lessons exactly the "human knowledge engineering" the lesson warns about — destined to be subsumed by the next model generation, making the curation effort a depreciating asset?

Two answers, one conceptual and one instrumental. Conceptually, the bitter lesson is about *capability*: chess search, speech features, vision pipelines — things a scaled learner can rediscover from data because a ground truth exists to converge on. An organization's conventions are not capability but *preference*: when two designs are both valid, which one this org chose. No amount of compute infers a boundary condition that exists only inside the organization; scale can subsume the *technique* tier of the corpus but not the *taste* tier, because taste is not in any training distribution the model will ever see (private repos, unwritten conventions, this quarter's architectural decision). SkillsBench's domain gradient is the empirical trace of exactly this line: lifts shrink where pretraining coverage grows [31]. Instrumentally, the design *agrees* with the bitter lesson rather than fighting it: the base-model redundancy probe (Section 15.4) is a bitter-lesson detector — any learning the new model generation subsumes is flagged and retired, so the corpus continuously sheds whatever scale has absorbed and converges toward the irreducibly org-specific residue. If Sutton is fully right about some learning class, the probe's job is to prove it, per-learning, and empty that class out.

### 13.3 The long-context objection

Context windows are growing faster than our corpus: why maintain a retrieval structure at all instead of loading everything, always? Three verified results answer. Position and length degrade in-context retrieval quality — models reliably under-use information in the middle of long contexts [38]. Injection without selection degrades outcomes: the lifecycle survey's audit includes a system showing that static all-injection of even a fixed library hurts, motivating adaptive selection and budgeting [5]. And more documentation is not better documentation: SkillsBench finds comprehensive prose essentially flat (+0.7pp) where focused, minimal skill modules deliver +19.0 to +21.5pp [31]. The cost argument compounds the quality one: an always-on corpus taxes every session on every machine (Section 7), and the tax is paid in attention as well as tokens. Long context changes the *budget*, not the *economics* — selection under a budget remains the problem, which is why the design's unit is the minimal sufficient subtree, not the whole tree.
...
4. **Reviewing text is not reviewing behavior.** The gate's verdict is a judgment about prose whose runtime effect is prompt text interpreted by a model (Section 9.1). Adversarial or merely misleading proposals are within the threat model; automated proposal screening does not yet exist.
5. **External validity is one organization, and part of the evidence base is preprint-stage.** All evidence will initially come from a single small org with a particular stack and review culture; the neighbor systems in the matrix are moving targets verified at a point in time; and several of the 2026 empirical anchors (SkillsBench, SkillGenBench, Ratchet, ASSAY) are arXiv preprints not yet peer-reviewed — their convergence with one another is reassuring but not a substitute (the two surveys have since been accepted: the lifecycle survey at TMLR, the memory survey at ACL Findings). All references were verified against their sources in July 2026; benchmark figures cite the paper versions current at that date. The public reusable-workflow distribution model is the replication path, not a result.
...
4. **Base-model redundancy probe, retirement, and the attic** (build): for each learning, measure whether the model already complies *without* the memory loaded. Tests the "absent from the weights" assumption per-learning (Section 11), supplies a retirement signal (a learning the base model now subsumes is dead weight), and cuts both ways on model transitions — a new model may subsume learnings the old one needed, and may *lack* behavior an old learning enforced, so un-retirement is first-class. Design: retirement moves a learning to an attic (tombstoned with a *reason* — `retired-unused` versus `retired-wrong`), never deletes; on a model transition the redundancy probe re-runs over the attic and failures become revival candidates, proposed as ordinary proposal PRs. Capture passes consult the attic before authoring: a proposal matching a `retired-unused` entry becomes a revival proposal instead of a duplicate, and a proposal matching a `retired-wrong` entry must address the recorded reason — the attic doubles as negative knowledge.
5. **Paired hot-versus-cold capture** (build): same PR, embedded capture pass versus post-hoc transcript pass; compare proposal quality, acceptance, and yield. Grounds or kills Section 8's "strictly more signal" claim.
6. **The gating checks of Section 6.2 as PR statuses** (origin-firing done-v0; the rest build): the origin-firing check ships as a PR status on the knowledge repo — a single agent pass is handed the proposal's own index plus the origin thread (recovered from the proposal's `learned-from:` provenance marker) replayed as a fresh task, and the status fails when a changed learning is not among the learnings the pass declares it would load; structure-only diffs and unmarked (migration/manual) PRs skip green. The check operationalizes encoding specificity ([16]; Section 2.3): it verifies that the memory is retrievable in the very context that produced it. The v0 is deliberately single-shot — cue firing is stochastic (Section 6.2), so a marginal cue can flake and a re-run is the second opinion; the statistical form (n replicates, TOST margins, flakiness policy) arrives with the retrieval-equivalence suite for structure diffs, which remains the structure-gate graduation mechanism and the model-transition regression suite, alongside the non-inferiority outcome check for content diffs, small per-PR and full nightly.
7. **Tree-model deliverables** (build): the sub-index convention and loader; per-tier token-budget instrumentation; *budget discovery* — the cue budget is not declared but measured, as the empirically observed list length where selection quality degrades (our own version of the survey's tens-to-hundreds threshold; requires aggregated firing telemetry, sections large enough to stress, and the masking harness — until then budgets are labeled provisional heuristics); split criteria (split a section when its cue list exceeds the budget its firing rate justifies, with masking-harness edge weights); `/learn --reindex` emitting the re-shaped tree as a proposal-PR diff (Section 7.4).
...
10. **Graduation-ladder instrumentation** (build): proposal acceptance-rate telemetry (rung-2 prerequisite); regression observability with provenance attribution and the documented demote/re-graduate runbook — the re-entrancy protocol of Section 6.3 made operational, including model-controlled capture re-runs (Section 12's observation).
11. **Roadmap-aware capture and generation** (build): the org's plans repository is a forecast of future tasks — and, in this design, itself a gate-curated artifact (plans are canonized through review), a *prospective* counterpart to the retrospective corpus. The boundary is explicit: the reward-model mapping of Section 4 applies only to the retrospective half — learnings are distilled preferences; plans are governed *context*, not preference distillation. Two consumers, both future work. *Capture*: feed the roadmap into the capture rubric, converting task-agnostic distillation (the case where generators fail [30]) into semi-task-conditioned distillation — prefer learnings relevant to upcoming work, deprioritize retirement candidates attached to retired subsystems; a capture-policy experiment: acceptance and usefulness of roadmap-weighted versus unweighted capture. *Generation*: inject open-plan constraints at build time and flag diff-versus-plan conflicts as a check — motivated by a live near-miss (July 2026: a PR introduced a workflow that an open plan had already pinned the runner group against; the human gate caught it, and no learning could have, since nothing had yet gone wrong to distill). Instrumented as a retrodiction probe: on historical PR-plus-plan pairs, does the check flag the known interaction?

Falsifiable predictions this agenda can refute: (i) curated org-specific learnings produce a measurably positive per-learning causal contribution where the literature reports zero-to-negative aggregate benefit for self-generated corpora; (ii) retrieval quality degrades measurably as the flat index grows, and tiered splits recover it; (iii) reindex diffs pass retrieval-equivalence at high rates, validating structure-gate graduation; (iv) on a model transition, the probe suite detects cue-firing changes that would otherwise surface as silent behavior shifts; (v) taste-tier learnings show low base-model redundancy while technique-tier learnings show higher redundancy; (vi) embedded (hot) capture outperforms post-hoc transcript capture on proposal acceptance at equal volume.
...
[2] Zhou et al., "Externalization in LLM Agents: A Unified Review of Memory, Skills, Protocols and Harness Engineering" (2026), arXiv:2604.08224.
...
[5] Li, "Dynamic Agent Skills: A Lifecycle Survey and Taxonomy of Evolving Skill Libraries," TMLR 2026, arXiv:2607.10113.
[6] Xu & Yan, "Agent Skills for Large Language Models: Architecture, Acquisition, Security, and the Path Forward" (2026), arXiv:2602.12430.
...
[12] DeepSeek-AI, "DeepSeek-R1: Incentivizing Reasoning Capability in LLMs via Reinforcement Learning" (2025), arXiv:2501.12948; journal version in Nature (2025).
[13] Luo et al., "From Storage to Experience: A Survey on the Evolution of LLM Agent Memory Mechanisms," Findings of ACL 2026, arXiv:2605.06716.
...
[23] Wang et al., "Not All Skills Help: Measuring and Repairing Agent Knowledge" (ASSAY, 2026), arXiv:2606.15390.
[24] Zhang et al., "Ratchet: A Minimal Hygiene Recipe for Self-Evolving LLM Agents" (2026), arXiv:2605.22148.
...
[28] Chishti, Oyinloye & Li, "Test Before You Deploy: Governing Updates in the LLM Supply Chain," LLMSC Workshop @ FSE 2026, arXiv:2604.27789.
...
[35] Leshin et al., "Behavioral Fingerprints for LLM Endpoint Stability and Identity" (2026), arXiv:2603.19022.
...
[40] Sutton, "The Bitter Lesson" (2019), incompleteideas.net/IncIdeas/BitterLesson.html.
[41] Nader, Schafe & Le Doux, "Fear Memories Require Protein Synthesis in the Amygdala for Reconsolidation after Retrieval," Nature 406:6797 (2000).