...
6. **The gating checks of Section 6.2 as PR statuses** (origin-firing done-v0; the rest build): the origin-firing check ships as a PR status on the knowledge repo — a single agent pass is handed the draft's own index plus the origin thread (recovered from the draft's `learned-from:` provenance marker) replayed as a fresh task, and the status fails when a changed learning is not among the learnings the pass declares it would load; structure-only diffs and unmarked (migration/manual) PRs skip green. The v0 is deliberately single-shot — cue firing is stochastic (Section 6.2), so a marginal cue can flake and a re-run is the second opinion; the statistical form (n replicates, TOST margins, flakiness policy) arrives with the retrieval-equivalence suite for structure diffs, which remains the structure-gate graduation mechanism and the model-transition regression suite, alongside the non-inferiority outcome check for content diffs, small per-PR and full nightly.
